...
function ContentBlock({ block }: { block: RulesBlock }) {
  if (block.type === 'text') return (
    <p style={{ fontSize: '14px', color: 'var(--text-dim)', margin: '0 0 8px', lineHeight: 1.6 }}>
      {block.text}
    </p>
  )
  ...
  if (block.type === 'list') return (
    <ul style={{ margin: '0 0 8px', paddingLeft: '20px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
      {block.items.map((item, i) => (
        <li key={i} style={{ fontSize: '14px', color: 'var(--text-dim)', lineHeight: 1.5 }}>{item}</li>
      ))}
    </ul>
  )
        ...
        <tbody>
          {block.rows.map((row, i) => (
            ...
            >
              {row.map((cell, j) => (
                <td key={j} style={{
                  padding: '8px 12px', color: j === 0 ? 'var(--text)' : 'var(--text-dim)',
                  fontWeight: j === 0 ? 600 : 400, lineHeight: 1.4,
                  verticalAlign: 'top',
                }}>{cell}</td>
              ))}
            </tr>
...
function RulesSection({ section, accentColor }: { section: SystemRules['sections'][0]; accentColor: string }) {
  ...
  return (
    <div style={{
      ...
      ...(open ? { borderColor: `${accentColor}44` } : {}),
    }}>
      <button
        onClick={() => setOpen(v => !v)}
        style={{
          ...
          color: open ? accentColor : 'var(--text)', textAlign: 'left',
...
export default function RulesPage() {
  const [activeSystem, setActiveSystem] = useState<string>('fate-core')
  const system = ALL_SYSTEM_RULES.find(s => s.id === activeSystem)!
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', paddingBottom: '32px' }}>
      ...
      <div>
        ...
        <p style={{ fontSize: '13px', color: 'var(--text-muted)', margin: 0 }}>
          Краткий справочник по механикам
        </p>
      </div>
      ...
      <div style={{
        ...
        background: 'var(--surface)', border: `1px solid ${system.accentColor}33`,
        borderLeft: `3px solid ${system.accentColor}`,
      }}>
        <h2 style={{ fontFamily: 'Cinzel, serif', fontSize: '18px', fontWeight: 700, color: system.accentColor, margin: '0 0 4px' }}>
          {system.name}
        </h2>
      ...
      <ExpandAllControls system={system} />
...
// ── Контрол развернуть/свернуть все ────────────────────────────────────
// Вынесен отдельно чтобы не усложнять основной компонент
function ExpandAllControls({ system }: { system: SystemRules }) {
  return (
    <div style={{ display: 'flex', gap: '8px' }}>
      <span style={{ fontSize: '12px', color: 'var(--text-muted)', alignSelf: 'center' }}>
        {system.sections.length} разделов
      </span>
    </div>
  )
}
