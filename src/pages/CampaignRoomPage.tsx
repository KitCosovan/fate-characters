...
function PlayerCard({ character, ownerName, isMe, onClick }: {
  ...
  const totalStress = character.stressTracks.reduce((s, tr) => s + tr.boxes.length, 0)
  const usedStress  = character.stressTracks.reduce((s, tr) => s + tr.boxes.filter(b => b.checked).length, 0)
    ...
    <div onClick={onClick} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px', flexShrink: 0 }}>
          {isMe && <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 8px', borderRadius: '20px', background: 'var(--accent)', color: 'var(--bg)' }}>{t('room.my_badge')}</span>}
          <span style={{ fontSize: '10px', color: 'var(--text-muted)', maxWidth: '120px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ownerName}</span>
        </div>
      </div>
      {totalStress > 0 && (
        ...
        {isGm && (
          <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
            ...
            <button onClick={onToggleVisibility} style={{
              ...
              {isVisible ? `👁 ${t('room.npc_visible')}` : `🙈 ${t('room.npc_hidden')}`}
            </button>
          </div>
        )}
      </div>
...
function NpcFieldsModal({ character, onClose, onSave }: {
  ...
  const fieldKey: Record<NpcVisibleField, string> = {
    concept: 'room.field_concept', aspects: 'room.field_aspects',
    skills: 'room.field_skills', stress: 'room.field_stress',
    consequences: 'room.field_consequences', stunts: 'room.field_stunts',
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h3 style={{ fontFamily: 'Cinzel, serif', fontSize: '16px', fontWeight: 700, color: 'var(--text)', margin: '0 0 16px' }}>
          {t('room.npc_fields_title')} — {character.name}
        </h3>
...
export default function CampaignRoomPage() {
  ...
  const { members, characters, myRole, loading, joinRoom, leaveRoom, setNpcVisibility, setNpcVisibleFields } = useCampaignRoomStore()
  ...
  const handleGenerateInvite = async () => {
    ...
    await supabase.from('campaigns').update({ user_id: user.id }).eq('id', campaignId).is('user_id', null)
    const code = Math.random().toString(36).slice(2, 8).toUpperCase()
    const { error } = await supabase.from('campaigns').update({ invite_code: code }).eq('id', campaignId)
    if (!error) {
  ...
  const getOwnerName = (ownerId?: string) => {
    if (!ownerId) return '—'
    if (ownerId === user?.id) return t('room.my_badge')
    return members.find(m => m.userId === ownerId)?.displayName ?? t('room.role_player')
  }

  if (!user) return (
    <div style={{ textAlign: 'center', padding: '64px', color: 'var(--text-muted)' }}>
      {t('room.npc_no_access')}
    </div>
  )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', padding: '24px', paddingBottom: '48px', maxWidth: '720px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '2px', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                {members.map(m => (
                  <span key={m.id} style={{
                    ...
                    background: 'var(--surface-2)', color: m.role === 'gm' ? '#c8a96e' : '#70a0e0',
                  }}>
                    {m.userId === user.id ? t('room.my_badge') : (m.displayName ?? t('room.role_player'))}
                    {m.role === 'gm' ? ' ⚔️' : ''}
                  </span>
                ))}
              </div>
            ...
            {myRole && (
              <span style={{
                ...
                {isGm ? `⚔️ ${t('room.role_gm')}` : `🎲 ${t('room.role_player')}`}
              </span>
            )}
          </div>
        </div>
        {isGm && (
          ...
          >
            <IconShare size={16} />
            {inviteLoading ? t('room.creating_invite') : inviteCode ? t('room.invite') : t('room.create_invite')}
          </button>
        )}
      </div>
      ...
      {showInvite && inviteCode && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ flex: 1, background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: '10px', padding: '14px 16px', textAlign: 'center' }}>
            ...
            <button onClick={copyInvite} style={{
              ...
              {copied ? `✓ ${t('room.copied')}` : `🔗 ${t('room.copy_link')}`}
            </button>
          </div>
      ...
      {loading && <p style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '14px' }}>{t('room.loading')}</p>}

      {/* Персонажи */}
      {playerChars.length > 0 && (
        <section style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <h2 style={{ fontFamily: 'Cinzel, serif', fontSize: '13px', fontWeight: 700, color: 'var(--accent)', letterSpacing: '0.1em', textTransform: 'uppercase', margin: 0, paddingBottom: '10px', borderBottom: '1px solid var(--border)' }}>
            {t('room.players_section', { count: playerChars.length })}
          </h2>
          {playerChars.map(c => (
            <PlayerCard key={c.id} character={c} ownerName={getOwnerName(c.ownerId)}
              isMe={c.ownerId === user.id} onClick={() => navigate(`/character/${c.id}`)} />
          ))}
        </section>
      )}
      ...
      {(isGm ? npcChars : visibleNpcs).length > 0 && (
        <section style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <h2 style={{ fontFamily: 'Cinzel, serif', fontSize: '13px', fontWeight: 700, color: 'var(--accent)', letterSpacing: '0.1em', textTransform: 'uppercase', margin: 0, paddingBottom: '10px', borderBottom: '1px solid var(--border)' }}>
            {t('room.npcs_section', { count: isGm ? npcChars.length : visibleNpcs.length })}
            {isGm && npcChars.length > visibleNpcs.length && (
              <span style={{ fontSize: '11px', fontWeight: 400, color: 'var(--text-muted)', marginLeft: '8px', textTransform: 'none' }}>
                {t('room.hidden_count', { count: npcChars.length - visibleNpcs.length })}
              </span>
            )}
          </h2>
      ...
      {!loading && playerChars.length === 0 && npcChars.length === 0 && (
        <div style={{ textAlign: 'center', padding: '64px 24px', color: 'var(--text-muted)' }}>
          ...
          <p style={{ fontSize: '13px', margin: 0 }}>
            {isGm ? t('room.empty_hint_gm') : t('room.empty_hint_player')}
          </p>
        </div>
      )}
