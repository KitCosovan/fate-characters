...
export default function JoinCampaignPage() {
  ...
  const handleJoin = async (joinCode = code) => {
    ...
    if (!joinCode.trim()) { setError(t('join.not_found')); return }
    setLoading(true); setError(null)
    const nameToUse = displayName.trim() || user.email || t('room.role_player')
  ...
  const inputStyle: React.CSSProperties = {
    ...
    borderRadius: '12px', padding: '12px 14px', fontSize: '15px', color: 'var(--input-text)',
      ...
      <div style={{ width: '100%', maxWidth: '380px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '20px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <label style={labelStyle}>{t('join.code_label')}</label>
            <input
              value={code} onChange={e => setCode(e.target.value.toUpperCase())}
              placeholder={t('join.code_placeholder')} maxLength={6}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <label style={labelStyle}>
              {t('join.name_label')} <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>{t('join.name_optional')}</span>
            </label>
            <input
              value={displayName} onChange={e => setName(e.target.value)}
              placeholder={user?.email ?? t('join.name_label')}
              onKeyDown={e => e.key === 'Enter' && handleJoin()}
              style={inputStyle}
              onFocus={e => { e.target.style.borderColor = 'var(--input-border-focus)' }}
              onBlur={e => { e.target.style.borderColor = 'var(--input-border)' }}
          ...
          <button onClick={() => handleJoin()} disabled={loading} style={{
            background: 'var(--accent)', color: 'var(--bg)', border: 'none', borderRadius: '12px',
            padding: '13px', cursor: loading ? 'default' : 'pointer', fontSize: '15px',
            fontWeight: 700, fontFamily: 'DM Sans, sans-serif', opacity: loading ? 0.7 : 1, transition: 'opacity 0.15s',
          }}>
            {loading ? t('join.submitting') : t('join.submit')}
          </button>
        </div>

        <button onClick={() => navigate(-1)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: '13px', fontFamily: 'DM Sans, sans-serif', textAlign: 'center' }}>
          {t('join.back')}
        </button>
      </div>
    </div>
