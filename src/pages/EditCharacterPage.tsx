...
export default function EditCharacterPage() {
  ...
  const handleSave = () => {
    if (!character.name.trim()) { alert(t('edit_character.no_name')); return }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', paddingBottom: '32px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        <button onClick={() => navigate(`/character/${character.id}`)} style={{
        ...
        <div>
          <h1 style={{ fontFamily: 'Cinzel, serif', fontSize: '20px', fontWeight: 700, color: 'var(--text)', margin: 0 }}>
            {t('edit_character.title')}
          </h1>
          <p style={{ fontSize: '12px', color: 'var(--text-muted)', margin: 0 }}>{config.name}</p>
        </div>
      </div>

      <Input label={t('create_character.name_label')} value={character.name} onChange={e => update({ name: e.target.value })} />
      <Divider />
      ...
      <SkillAdvancement skills={config.skills} selected={character.skills} onChange={skills => update({ skills })} />
      <Divider />
      ...
      <Button size="lg" onClick={handleSave}>{t('edit_character.save')}</Button>
    </div>
  )
}
