...
export default function CreateCharacterPage() {
  ...
  const handleSave = () => {
    if (!character.name.trim()) { alert(t('create_character.no_name')); return }
  ...
  return (
    <div className="flex flex-col gap-6 pb-8">
      ...
      <Input
        label={t('create_character.name_label')}
        placeholder={t('create_character.name_placeholder')}
      ...
      {config.hasScars && (
        <>
          <ScarsSection scars={character.scars} maxScars={config.maxScars ?? 3} onChange={scars => update({ scars })} />
          <Divider />
        </>
      )}
      ...
      {config.hasEquipment && (
        <>
          <EquipmentSection equipment={character.equipment} totalSlots={config.equipmentSlots ?? 6} onChange={equipment => update({ equipment })} />
          <Divider />
        </>
      )}

      <RefreshSection
        refresh={character.refresh} currentFatePoints={character.currentFatePoints}
        onRefreshChange={refresh => update({ refresh })} onFatePointsChange={currentFatePoints => update({ currentFatePoints })}
      />
      <Divider />
