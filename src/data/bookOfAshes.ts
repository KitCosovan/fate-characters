// src/data/bookOfAshes.ts
// Базовый конфиг на русском — локализованные метки добавляются через useLocalizedConfig
...
export const bookOfAshesConfig: SystemConfig = {
  ...
  aspectSlots: [
    { id: 'concept',    label: 'Концепция',       required: true,  placeholder: 'Кто этот человек и почему он в Походе Пепла?' },
    { id: 'vice',       label: 'Порок',            required: true,  placeholder: 'Внутренняя слабость или тёмная сторона персонажа' },
    { id: 'bright-side',label: 'Светлая сторона', required: true,  placeholder: 'То хорошее, что не даёт окончательно сломаться' },
    { id: 'phase-1',    label: 'Аспект из фазы 1',required: false, placeholder: 'Из твоего приключения' },
    { id: 'phase-2',    label: 'Аспект из фазы 2',required: false, placeholder: 'Из чужого приключения, где ты появился' },
  ],
  skills: [
