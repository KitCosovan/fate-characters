...
export default function BottomNav() {
  ...
  const showNav = ['/', '/encyclopedia', '/rules', '/faq'].includes(location.pathname)
    || location.pathname === '/'
  ...
  const MORE_ITEMS = [
    {
      path: '/encyclopedia', label: 'Энциклопедия',
    ...
    <>
      ...
      <nav className="bottom-nav" style={{
        ...
        <button onClick={() => setShowMore(v => !v)} style={{
          ...
          <span style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '0.04em' }}>Ещё</span>
        </button>
      </nav>
    </>
  )
