// src/components/character/StressTrack.tsx
// No localizable strings — no changes needed
...
export default function StressTrack({ track, label, onChange }: StressTrackProps) {
  const toggle = (index: number) => {
    const boxes: StressBox[] = track.boxes.map(b => b.index === index ? { ...b, checked: !b.checked } : b)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        {track.boxes.map(box => (
          <button key={box.index} onClick={() => toggle(box.index)} style={{
            ...
            onMouseEnter={e => { if (!box.checked) (e.currentTarget as HTMLElement).style.borderColor = 'var(--border-accent)' }}
            onMouseLeave={e => { if (!box.checked) (e.currentTarget as HTMLElement).style.borderColor = 'var(--border)' }}
          >
            {box.index + 1}
          </button>
        ))}
