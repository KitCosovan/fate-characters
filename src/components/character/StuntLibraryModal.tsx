...
export default function StuntLibraryModal({ isOpen, onClose, onSelect, systemId }: StuntLibraryModalProps) {
  ...
  const handleSelect = (stunt: LibraryStunt) => {
    onSelect(stunt)
    onClose()
  }
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input
            ...
            style={{
              ...
              padding: '8px 12px 8px 30px', fontSize: '13px',
        ...
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
          ...
          {skills.map(skill => {
            const count = stuntLibrary.filter(s =>
              s.skillId === skill.id &&
          ...
          {Array.from(grouped.entries()).map(([skillId, stunts]) => (
            <div key={skillId} style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              {stunts.map(stunt => (
                ...
                >
                  <p style={{ fontWeight: 700, fontSize: '13px', color: 'var(--text)', margin: '0 0 4px' }}>
                    {stunt.name}
                  </p>
