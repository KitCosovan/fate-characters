// src/components/character/RefreshSection.tsx
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{
        ...
        display: 'flex', justifyContent: 'space-around', gap: '16px',
      }}>
        <Counter label="Refresh" value={refresh} min={1} onChange={onRefreshChange} />
        <div style={{ width: '1px', background: 'var(--border)' }} />
        <Counter label={t('refresh.current')} value={currentFatePoints} min={0} onChange={onFatePointsChange} />
      </div>
    </div>
  )
}
...
function Counter({ label, value, min, onChange }: { label: string; value: number; min: number; onChange: (v: number) => void }) {
  const btnStyle = {
    ...
    background: 'var(--surface-3)', color: 'var(--text)', fontSize: '16px', cursor: 'pointer',
    display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'DM Sans, sans-serif',
  }
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
      <span style={{ fontSize: '12px', color: 'var(--text-dim)', fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase' as const }}>
        {label}
      </span>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        <button onClick={() => onChange(Math.max(min, value - 1))} style={btnStyle}>−</button>
        <span style={{ fontSize: '28px', fontWeight: 700, fontFamily: 'Cinzel, serif', color: 'var(--accent)', minWidth: '32px', textAlign: 'center' as const }}>
          {value}
        </span>
